...
<title>Golfeados · Golf Club</title>
...
<div id="loginScreen" style="display:none;">
  <div class="login-card" style="max-width:400px;">
    ...
    <div class="login-logo">
      ...
      <div class="login-sub">Golf Club · Tournament Manager</div>
    </div>
    ...
    <div id="loginView">
      ...
      <div class="field">
        <label>Email, usuario o teléfono</label>
      ...
      <div class="field">
        ...
        <div class="input-icon-wrap">
          ...
          <button class="toggle-pass" type="button" onclick="togglePass('loginPassword',this)">👁</button>
        </div>
      </div>
      ...
      <button class="btn-link" onclick="showReset()">¿Olvidaste tu contraseña?</button>
    </div>
    ...
    <div id="registerView" style="display:none;">
      ...
      <div class="field">
        ...
        <div id="usernameHint" style="font-size:11px;color:var(--muted);margin-top:4px;">Solo letras, números y guión bajo</div>
      </div>
      <div class="field">
        <label>Email *</label>
      ...
      <div class="field">
        <label>Teléfono (opcional pero recomendado)</label>
      ...
      <div class="field">
        <label>Contraseña *</label>
        <div class="input-icon-wrap">
          <input type="password" id="regPassword" placeholder="Mín. 6 caracteres" oninput="checkStrength(this)"/>
          <button class="toggle-pass" type="button" onclick="togglePass('regPassword',this)">👁</button>
        </div>
      ...
      <button class="btn-login" id="regBtn" onclick="handleRegister()">Continuar →</button>
      <div class="login-or">o regístrate con</div>
      <button class="btn-google" onclick="handleGoogleSignIn()">
        <svg width="18" height="18" viewBox="0 0 48 48"><path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.08 17.74 9.5 24 9.5z"/><path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/><path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"/><path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.18 1.48-4.97 2.35-8.16 2.35-6.26 0-11.57-3.59-13.46-8.83l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/></svg>
        Continuar con Google
      </button>
      <button class="btn-link" onclick="showLogin()">← Ya tengo cuenta</button>
    </div>
    ...
    <div id="completarPerfilView" style="display:none;">
      <div style="text-align:center;margin-bottom:20px;">
        ...
        <div style="font-size:16px;font-weight:700;color:var(--green);">¡Bienvenido al club!</div>
        <div style="font-size:12px;color:var(--muted);margin-top:6px;">Un paso más para completar tu perfil</div>
      </div>
      <div id="completarPerfilError" class="error-box" style="display:none;margin-bottom:12px;"></div>
      <div class="field">
        <label>Nombre completo</label>
        <input type="text" id="completarNombre" placeholder="Ej: Jesús Aguiar"/>
      </div>
      <div class="field">
        <label>Nombre de usuario *</label>
        <div class="input-icon-wrap">
        ...
        <div style="font-size:11px;color:var(--muted);margin-top:4px;">Solo letras, números y guión bajo</div>
      </div>
      <div class="field">
      ...
      <button class="btn-login" id="completarBtn" onclick="handleCompletarPerfil()">Entrar al Club 🏌️</button>
    </div>
    ...
    <div id="verifyChoiceView" style="display:none;">
      ...
      <div class="verify-option mb-12" id="optEmail" onclick="chooseVerify('email')">
        ...
        <div>
          ...
          <div id="verifyEmailAddr" style="font-size:12px;color:var(--muted);">Gratis e instantáneo</div>
        </div>
      </div>
      <div class="verify-option" id="optSMS" onclick="chooseVerify('sms')">
        ...
        <div>
          ...
          <div id="verifyPhoneNum" style="font-size:12px;color:var(--muted);">Recibirás un código de 6 dígitos</div>
        </div>
      </div>
      <div id="recaptcha-container" style="margin-top:12px;"></div>
      <button class="btn-login" style="margin-top:16px;" id="btnSendVerify" onclick="sendVerification()">Enviar código</button>
      <button class="btn-link" onclick="skipVerify()">Verificar más tarde</button>
    </div>
    ...
    <div id="smsCodeView" style="display:none;">
      <div style="text-align:center;margin-bottom:20px;">
        ...
        <div style="font-size:15px;font-weight:700;color:var(--green);">Código enviado</div>
      ...
      <button class="btn-login" id="btnConfirmSMS" onclick="confirmSMS()">Confirmar código</button>
      <button class="btn-link" onclick="showVerifyChoice()">← Cambiar método</button>
      <button class="btn-link" id="btnResendSMS" onclick="resendSMS()" style="display:none;">Reenviar código</button>
    </div>
    ...
    <div id="emailSentView" style="display:none;">
      <div style="text-align:center;padding:10px 0 20px;">
        ...
        <div style="font-size:16px;font-weight:700;color:var(--green);">¡Revisa tu email!</div>
        <div id="emailSentAddr" style="font-size:13px;color:var(--muted);margin-top:6px;line-height:1.5;"></div>
      </div>
      <button class="btn-login" onclick="recheckEmail()">Ya verifiqué mi email ✓</button>
      <button class="btn-link" onclick="resendEmail()">Reenviar email</button>
      <button class="btn-link" onclick="skipVerify()">Verificar más tarde</button>
    </div>

    <!-- ⑥ VERIFICACIÓN PENDIENTE (usuario ya registrado pero no verificó) -->
    <!-- VINCULAR CUENTA CON CÓDIGO DE INVITACIÓN -->
    <div id="vincularView" style="display:none;">
      <div style="text-align:center;padding:10px 0 20px;">
        <div style="font-size:48px;margin-bottom:12px;">⛳</div>
        <div style="font-size:16px;font-weight:700;color:var(--green);margin-bottom:8px;">¡Bienvenido al club!</div>
        <div style="font-size:13px;color:var(--muted);line-height:1.5;">Si el administrador ya cargó tu perfil de jugador,<br/>ingresa tu código de invitación para vincularte.</div>
      </div>
      ...
      <button class="btn-login" id="vincularBtn" onclick="handleVincular()">Vincular mi perfil 🏌️</button>
      <button class="btn-link" style="margin-top:10px;" onclick="skipVincular()">Saltar por ahora →</button>
    </div>

    <div id="pendingVerifyView" style="display:none;">
      <div style="text-align:center;padding:10px 0 20px;">
        <div style="font-size:48px;margin-bottom:12px;">⚠️</div>
        <div style="font-size:15px;font-weight:700;color:var(--gold);">Cuenta pendiente de verificación</div>
        <div style="font-size:13px;color:var(--muted);margin-top:8px;line-height:1.5;">Debes verificar tu email o teléfono para acceder.</div>
      </div>
    ...
    <div id="resetView" style="display:none;">
      <div style="font-size:13px;color:var(--muted);margin-bottom:14px;">Ingresa tu email para recibir un enlace de recuperación.</div>
      ...
      <div id="resetSuccess" style="display:none;text-align:center;padding:20px 0;">
        ...
        <div style="color:var(--text);font-size:14px;">Email de recuperación enviado.</div>
      </div>
      <div id="resetForm">
      ...
      <button class="btn-link" onclick="showLogin()">← Volver al login</button>
    </div>

  </div>
  <div class="login-footer">PLAY · RELAX · ENJOY · SINCE 2024</div>
</div>
...
<div id="updateBanner" style="display:none;position:fixed;bottom:0;left:0;right:0;z-index:500;
  ...
  <div>
    <div style="color:#fff;font-weight:700;font-size:14px;">🔄 Nueva versión disponible</div>
    <div style="color:rgba(255,255,255,0.8);font-size:12px;margin-top:2px;" id="updateBannerSub">Nueva versión disponible</div>
  </div>
...
<div id="app" style="display:none;">
  <div id="header">
    ...
    <div class="header-right">
      ...
      <div id="connPill" title="Estado de conexión" style="display:flex;align-items:center;padding:4px;border-radius:50%;background:transparent;transition:all 0.4s;">
        <span id="connDot" style="width:8px;height:8px;border-radius:50%;background:#4CAF50;display:inline-block;"></span>
      </div>
      <button id="headerUser" onclick="openPerfilModal()" style="background:var(--cardL);border:1px solid var(--border);border-radius:8px;padding:4px 12px;font-size:12px;font-weight:700;color:var(--text);cursor:pointer;max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"></button>
      <button class="btn-logout" onclick="handleLogout()">Salir</button>
    </div>
  </div>

  <div id="nav">
    <button class="nav-btn active" data-tab="dashboard" onclick="goTab('dashboard')">🏠 Inicio</button>
    <button class="nav-btn" data-tab="mistorneos" onclick="goTab('mistorneos')">🏆 Mis Torneos</button>
    <button class="nav-btn" data-tab="jornadas" onclick="goTab('jornadas')">🏌️ Partidas</button>
    <button class="nav-btn" data-tab="clubes" onclick="goTab('clubes')" id="navClubes" style="display:none;">⛳ Clubes</button>
  </div>
...
<div id="modalJornada" class="modal-overlay" style="display:none;" onclick="closeModalJornada()">
  <div class="modal-box" onclick="event.stopPropagation()">
    <div class="modal-header">
      ...
      <button onclick="closeModalJornada()" style="background:none;border:none;font-size:20px;color:var(--muted);cursor:pointer;">✕</button>
    </div>
        ...
        <div style="display:flex;flex-direction:column;gap:8px;">
          <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:var(--cardL);border-radius:10px;border:1px solid var(--border);">
            ...
            <div>
              ...
              <div style="font-size:11px;color:var(--muted);">Desactiva para partidas de práctica o exhibición (🚫)</div>
            </div>
          </div>
          <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:var(--cardL);border-radius:10px;border:1px solid var(--border);">
            ...
            <div>
              <label for="jornadaReglasEspeciales" style="font-weight:700;font-size:13px;cursor:pointer;">⚙️ Reglas especiales para esta partida</label>
          ...
          <div id="reglasEspecialesPanel" style="display:none;background:var(--cardL);border-radius:10px;padding:14px;border:1px solid var(--border);">
            <div class="text-11 text-muted font-bold" style="letter-spacing:1px;text-transform:uppercase;margin-bottom:10px;">Puntos por posición</div>
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:12px;">
              <div style="text-align:center;"><div style="font-size:16px;">🥇</div><input type="number" class="form-input" id="jrPts1" placeholder="4" min="0" style="text-align:center;padding:6px 4px;"/></div>
              <div style="text-align:center;"><div style="font-size:16px;">🥈</div><input type="number" class="form-input" id="jrPts2" placeholder="3" min="0" style="text-align:center;padding:6px 4px;"/></div>
              <div style="text-align:center;"><div style="font-size:16px;">🥉</div><input type="number" class="form-input" id="jrPts3" placeholder="2" min="0" style="text-align:center;padding:6px 4px;"/></div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
              <div><label class="form-label">✅ Bonus asistencia</label>
              ...
              <div><label class="form-label">❌ Penal no asistencia</label>
              ...
              <div><label class="form-label">🚫 Penal DQ</label>
...
<div id="modalClub" class="modal-overlay" style="display:none;" onclick="closeModalClub()">
  <div class="modal-box" onclick="event.stopPropagation()">
    <div class="modal-header">
      ...
      <button onclick="closeModalClub()" style="background:none;border:none;font-size:20px;color:var(--muted);cursor:pointer;">✕</button>
    </div>
    <div class="modal-body">
      ...
      <div style="display:flex;flex-direction:column;gap:14px;">
        ...
        <div>
          ...
          <div class="photo-upload-box" onclick="document.getElementById('clubFotoInput').click()">
            ...
            <div id="clubFotoPlaceholder">📷 Toca para subir foto</div>
...
<div id="modalJugador" class="modal-overlay" style="display:none;" onclick="closeModalJugador()">
  <div class="modal-box" onclick="event.stopPropagation()">
    <div class="modal-header">
      ...
      <button onclick="closeModalJugador()" style="background:none;border:none;font-size:20px;color:var(--muted);cursor:pointer;">✕</button>
    </div>
    <div class="modal-body">
      ...
      <div style="display:flex;flex-direction:column;gap:14px;">
        <div style="display:flex;align-items:center;gap:16px;">
          <div style="position:relative;flex-shrink:0;">
            ...
            <div id="jugadorFotoFallback" style="width:64px;height:64px;border-radius:50%;background:var(--green);display:flex;align-items:center;justify-content:center;color:#fff;font-weight:800;font-size:20px;">👤</div>
            <button type="button" onclick="document.getElementById('jugadorFotoInput').click()" 
              style="position:absolute;bottom:0;right:0;background:var(--green);border:none;border-radius:50%;width:22px;height:22px;color:#fff;font-size:12px;cursor:pointer;display:flex;align-items:center;justify-content:center;">✏️</button>
        ...
        <div>
          ...
          <div id="telefonoHint" style="font-size:11px;color:var(--muted);margin-top:4px;">Solo números, sin espacios ni guiones. Ej: 4141234567</div>
        </div>
      </div>
    </div>
    <div class="modal-footer">
...
<div id="modalTorneo" class="modal-overlay" style="display:none;" onclick="closeModalTorneo(event)">
  <div class="modal-box" onclick="event.stopPropagation()">
    <div class="modal-header">
      ...
      <button onclick="closeModalTorneo()" style="background:none;border:none;font-size:20px;color:var(--muted);cursor:pointer;">✕</button>
    </div>
    <div class="modal-body" style="display:flex;flex-direction:column;gap:16px;">
      ...
      <div style="background:var(--cardL);border-radius:10px;padding:14px;border:1px solid var(--border);">
        <div class="text-12 font-bold text-muted" style="letter-spacing:1px;text-transform:uppercase;margin-bottom:12px;">📋 Información</div>
        <div style="display:flex;flex-direction:column;gap:12px;">
          <!-- Logo del torneo -->
          <div>
            <label class="form-label">Logo del torneo (opcional)</label>
            <div style="display:flex;align-items:center;gap:12px;">
              <div id="tLogoPreviewBox" onclick="document.getElementById('tLogoInput').click()"
                style="width:56px;height:56px;border-radius:12px;background:var(--cardL);border:2px dashed var(--border);cursor:pointer;display:flex;align-items:center;justify-content:center;overflow:hidden;flex-shrink:0;">
                <img id="tLogoPreview" style="display:none;width:100%;height:100%;object-fit:cover;"/>
                <span id="tLogoPlaceholder" style="font-size:22px;">🏆</span>
              </div>
              <div style="flex:1;">
                <button type="button" onclick="document.getElementById('tLogoInput').click()" style="background:none;border:1px solid var(--border);border-radius:6px;padding:5px 12px;font-size:11px;color:var(--text);cursor:pointer;">Subir logo</button>
                <button type="button" id="tLogoClearBtn" onclick="clearTorneoLogo()" style="display:none;background:none;border:none;color:var(--muted);font-size:11px;cursor:pointer;margin-left:4px;">✕ Quitar</button>
              </div>
              <input type="file" id="tLogoInput" accept="image/*" style="display:none;" onchange="previewTorneoLogo(this)"/>
            </div>
            <div class="upload-progress"><div class="upload-progress-bar" id="tLogoProgress"></div></div>
          </div>
...
<!-- fechas: se gestionan por partidas -->
          <div style="display:flex;gap:12px;">
            <div style="flex:1;"><label class="form-label">N° Partidas</label>
          ...
          <div>
            ...
            <textarea class="form-input" id="tDescripcion" placeholder="Descripción del torneo..." rows="3" style="resize:vertical;"></textarea>
          </div>
                      <div>
            ...
            <div id="tDocExisting" style="display:none;margin-top:8px;display:flex;align-items:center;gap:8px;">
              <a id="tDocLink" href="#" target="_blank" style="font-size:12px;color:var(--blue);">📎 Ver documento actual</a>
              <button onclick="clearTorneoDoc()" style="background:none;border:none;color:var(--muted);font-size:12px;cursor:pointer;">✕ Quitar</button>
            </div>
          ...
          <div style="display:flex;gap:12px;">
            ...
            <div style="flex:1;"><label class="form-label">¿Quién carga?</label>
      ...
      <div style="background:var(--cardL);border-radius:10px;padding:14px;border:1px solid var(--border);">
        <div class="text-12 font-bold text-muted" style="letter-spacing:1px;text-transform:uppercase;margin-bottom:12px;">⚙️ Reglas de Puntuación</div>

        <!-- Puntos por posición -->
        <div style="margin-bottom:14px;">
          <label class="form-label">Puntos por posición</label>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px;">
          <div>
            <label class="form-label">✅ Bonus asistencia</label>
          ...
          <div>
            <label class="form-label">❌ Penal no asistencia</label>
            <div style="display:flex;align-items:center;gap:6px;">
          ...
          <div>
            <label class="form-label">🚫 Penal DQ</label>
            <div style="display:flex;align-items:center;gap:6px;">
          ...
          <div>
            <label class="form-label">🗑️ Descartes</label>
            <div style="display:flex;align-items:center;gap:6px;">
        ...
        <div>
          ...
          <select class="form-select" id="rEmpates" style="width:100%;margin-top:6px;" onchange="updateEmpateEjemplo()" oninput="updateEmpateEjemplo()">
            <option value="comparten">Comparten puntos — promedio de posiciones empatadas</option>
            <option value="posicion_anterior">Posición anterior — ambos toman la misma posición</option>
          </select>
      ...
      <div style="background:var(--cardL);border-radius:10px;padding:14px;border:1px solid var(--border);" id="seccionAdmins">
        <div class="text-12 font-bold text-muted" style="letter-spacing:1px;text-transform:uppercase;margin-bottom:12px;">👥 Co-Administradores</div>
...
<div id="agregarJugadorModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9999;align-items:flex-end;justify-content:center;" onclick="closeAgregarJugadorModal()">
  <div style="background:var(--white);border-radius:20px 20px 0 0;width:100%;max-width:500px;padding:24px 20px 32px;max-height:85vh;overflow-y:auto;" onclick="event.stopPropagation()">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
      ...
      <button onclick="closeAgregarJugadorModal()" style="background:none;border:none;font-size:22px;cursor:pointer;color:var(--muted);">✕</button>
    </div>
    <div style="display:flex;gap:8px;margin-bottom:16px;">
      <button id="tabBuscar" onclick="switchAgregarTab('buscar')"
        style="flex:1;padding:8px;border-radius:8px;border:2px solid var(--green);background:var(--green);color:#fff;font-weight:700;font-size:13px;cursor:pointer;">🔍 Buscar</button>
    ...
    <div id="panelBuscar">
      <input type="text" id="buscarJugadorInput" placeholder="Nombre, alias, teléfono o email..."
    ...
    <div id="panelCrear" style="display:none;">
      <div style="display:flex;flex-direction:column;gap:12px;">
        ...
        <div>
          ...
          <div style="display:flex;gap:8px;align-items:center;">
            <select class="form-select" id="newPaisCodigo" style="width:110px;">
              <option value="+58">🇻🇪 +58</option><option value="+1">🇺🇸 +1</option>
              <option value="+34">🇪🇸 +34</option><option value="+52">🇲🇽 +52</option>
              <option value="+57">🇨🇴 +57</option>
            </select>
        ...
        <button type="button" id="btnCrearJugador" class="btn-green" style="width:100%;padding:12px;font-size:14px;cursor:pointer;">Crear y agregar al torneo 🏌️</button>
      </div>
    </div>
  </div>
</div>
...
<div id="modalPerfil" class="modal-overlay" style="display:none;" onclick="closePerfilModal()">
  <div class="modal-box" onclick="event.stopPropagation()">
    <div class="modal-header">
      ...
      <button onclick="closePerfilModal()" style="background:none;border:none;font-size:20px;color:var(--muted);cursor:pointer;">✕</button>
    </div>
    <div class="modal-body" style="padding:20px 24px;">
      ...
      <div style="display:flex;flex-direction:column;gap:14px;">
        <div>
          ...
          <input type="text" class="form-input" id="perfilUsername" disabled style="background:var(--bg);color:var(--green);font-weight:700;"/>
        </div>
        <div>
        ...
        <div>
          ...
          <input type="text" class="form-input" id="perfilAlias" placeholder="Cómo te llaman en el campo"/>
        </div>
        <div>
      ...
      <!-- Cambiar contraseña (solo para email/password users) -->
      <div id="perfilPasswordSection" style="margin-top:20px;border-top:1px solid var(--border);padding-top:16px;">
        <div class="text-13 font-bold" style="margin-bottom:12px;">🔒 Cambiar contraseña</div>
        <div style="display:flex;flex-direction:column;gap:10px;">
          <input type="password" class="form-input" id="perfilPassActual" placeholder="Contraseña actual"/>
          <input type="password" class="form-input" id="perfilPassNueva" placeholder="Nueva contraseña (mín. 6 caracteres)"/>
          <input type="password" class="form-input" id="perfilPassConfirm" placeholder="Confirmar nueva contraseña"/>
        </div>
      </div>
      ...
      <div id="perfilVincularSection" style="margin-top:16px;border-top:1px solid var(--border);padding-top:16px;">
        <div class="text-13 font-bold" style="margin-bottom:8px;">🔑 Vincular código de invitación</div>
        <div class="text-11 text-muted" style="margin-bottom:10px;line-height:1.5;">Si un admin te dio un código para vincular un perfil existente con resultados, ingrésalo aquí.</div>
        <div style="display:flex;gap:8px;">
  ...
  <div class="card" style="margin-bottom:12px;overflow:hidden;">
    <div style="background:linear-gradient(135deg,var(--green),var(--teal));padding:16px 20px;cursor:pointer;"
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;">
        <div style="display:flex;align-items:center;gap:12px;">
          ${torneo.logoURL
            ?`<img src="${torneo.logoURL}" style="width:44px;height:44px;border-radius:10px;object-fit:cover;flex-shrink:0;border:2px solid rgba(255,255,255,0.3);"/>`
            :`<div style="width:44px;height:44px;border-radius:10px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0;">🏆</div>`}
          <div>
        ...
        ${myPos?`<div style="text-align:right;">
          <div style="color:rgba(255,255,255,0.7);font-size:10px;">Mi posición</div>
      ...
      <div style="display:flex;gap:20px;margin-top:12px;">
        ...
        <div><div style="color:rgba(255,255,255,0.6);font-size:9px;text-transform:uppercase;letter-spacing:1px;">Próxima</div>
          <div style="color:#fff;font-size:13px;font-weight:600;">${proxima?fmtFecha(proxima.fecha):'—'}</div></div>
      </div>
    </div>
